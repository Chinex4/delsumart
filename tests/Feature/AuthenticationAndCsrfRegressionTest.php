...
class AuthenticationAndCsrfRegressionTest extends TestCase
{
    ...
    {
        return User::create([
            'name' => 'Demo Student', 'matric_no' => 'DELSU/TEST/001',
            'email' => 'demo@example.test', 'programme' => 'Computing', 'level' => '200',
    ...
    {
        $this->post('/register', [
            'name' => 'New Student', 'matric_no' => 'DELSU/NEW/001',
            'email' => 'new@example.test', 'programme' => 'Computing', 'level' => '200',
            'password' => 'Password123!', 'password_confirmation' => 'Password123!',
        ])->assertRedirect('/verification');
        $this->assertAuthenticated();
        $this->get('/verification')->assertOk()->assertSee('Upload your documents');
    }
    ...
    public function test_password_login_requires_correct_one_time_code(): void
    {
        Notification::fake();
        $user = $this->user();
        $this->post('/login', ['login' => $user->email, 'password' => 'Password123!'])->assertRedirect('/mfa');
        $this->assertGuest();
        $notification = Notification::sent($user, MfaCodeNotification::class)->first();
        $this->post('/mfa', ['code' => 'wrong'])->assertSessionHasErrors('code');
        $this->assertGuest();
        $code = collect($notification->toMail($user)->introLines)
            ->first(fn ($line) => preg_match('/^\d{6}$/', $line));
    ...
    public function test_signed_webhook_is_not_blocked_by_browser_csrf_but_forms_are_protected(): void
    {
        ...
        $payload = json_encode(['event' => 'charge.success', 'data' => ['reference' => 'DM-CSRF']]);
        $this->call('POST', '/paystack/webhook', [], [], [], [
            'CONTENT_TYPE' => 'application/json',
            'HTTP_X_PAYSTACK_SIGNATURE' => hash_hmac('sha512', $payload, 'test-secret'),
        ], $payload)->assertOk();
