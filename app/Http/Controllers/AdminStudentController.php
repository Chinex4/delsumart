...
class AdminStudentController extends Controller  {
    public function index(Request $r) {
        ...
        if($s=$r->string('q')->trim()->value())$q->where(fn($x)=>$x->where('name','like',"%$s%")->orWhere('email','like',"%$s%")->orWhere('matric_no','like',"%$s%"));
        if($status=$r->string('status')->value())$q->whereHas('verification',fn($v)=>$v->where('verification_status',$status));
