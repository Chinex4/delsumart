...
use App\Models\{AuditLog,Dispute,FraudFlag,Listing,Transaction,User,Verification};
...
class AdminController extends Controller  {
    ...
    public function flags(Request $r) {
        ...
        if($level=$r->string('risk')->value())$level==='high'?$q->where('risk_score','>=',60):($level==='medium'?$q->whereBetween('risk_score',[30,59]):null);
    ...
    public function audits(Request $r) {
        ...
        if($action=$r->string('action')->trim()->value())$q->where('action_type','like',"%$action%");
