...
class PayoutController extends Controller
{
    ...
    public function verifyBankOtp(Request $request)
    {
        ...
        if (
            $record === null ||
            $record['attempts'] >= 5 ||
            Hash::check($data['otp'], $record['hash']) === false
        ) {
            if ($record) {
            ...
            throw ValidationException::withMessages([
                'otp' => 'The code is invalid or expired.',
            ]);
        }
        ...
        return back()->with(
            'success',
            'Security check passed. You can now set your payout account.',
        );
    }
    ...
    public function resolveBank(Request $request, PaystackService $paystack)
    {
        ...
        try {
            $resolved = $paystack->resolveAccount(
                $data['account_number'],
                $data['bank_code'],
            );
        ...
        $request
            ...
            ->updateOrCreate(
                ...
                [
                    ...
                    'paystack_recipient_code' => $recipient['recipient_code'] ?? null,
                    'verified_at' => now(),
                ],
            );
    ...
    public function store(Request $request)
    {
        ...
        DB::transaction(function () use ($user, $data) {
            $totalSales = (float) Transaction::where('seller_id', $user->id)
                ->where('status', 'released')
                ->lockForUpdate()
                ->sum('amount');
            $reserved = (float) PayoutRequest::where('user_id', $user->id)
                ->whereIn('status', ['pending', 'processing', 'paid'])
                ->lockForUpdate()
                ->sum('amount');
            ...
            if ($amount > max(0, $totalSales - $reserved)) {
                throw ValidationException::withMessages([
                    'amount' => 'This amount is higher than your available payout balance.',
                ]);
            }
            ...
            PayoutRequest::create([
                ...
                'amount' => $amount,
                'status' => 'pending',
                'reference' => 'PO-'.strtoupper(Str::random(14)),
            ]);
        });
    ...
    private function bankSessionValid(Request $request): bool
    {
        $verifiedAt = (int) $request
            ->session()
            ->get('payout_bank_verified_at', 0);

        return $verifiedAt > 0 &&
            $verifiedAt >= now()->subMinutes(15)->timestamp;
    }
}
