...
class ListingController extends Controller  {
    public function index(Request $r) {
        ...
        if($s=$r->string('q')->trim()->value())$q->where(fn($x)=>$x->where('title','like',"%$s%")->orWhere('description','like',"%$s%")->orWhere('category','like',"%$s%"));
        if($c=$r->string('category')->value())$q->where('category',$c);
        if($min=$r->integer('min_price'))$q->where('price','>=',$min);
        if($max=$r->integer('max_price'))$q->where('price','<=',$max);
    ...
    public function store(Request $r,FraudScoringService $fraud) {
        ...
        foreach($r->file('images',[]) as $i=>$image)$listing->images()->create(['path'=>$image->store('listings/'.$listing->id,'public'),'sort_order'=>$i]);
