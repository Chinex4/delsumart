...
class AuthController extends Controller
{
    public function register(Request $r)
    {
        ...
        $d['matric_no'] = strtoupper(
            preg_replace("/\s+/", '', trim($d['matric_no'])),
        );
        $u = User::create($d);
        Auth::login($u);
    ...
    public function login(Request $r, MfaService $mfa)
    {
        ...
        $field = filter_var($d['login'], FILTER_VALIDATE_EMAIL)
            ? 'email'
            : 'matric_no';
        $value =
            $field === 'matric_no'
                ? strtoupper(preg_replace("/\s+/", '', trim($d['login'])))
                : strtolower(trim($d['login']));
    ...
    public function verifyMfa(Request $r, MfaService $mfa)
    {
        ...
        if (! $mfa->verify($u, $d['code'])) {
            return back()->withErrors([
                'code' => 'The code is invalid, expired, or has exceeded its attempt limit.',
            ]);
        }
        Auth::login($u);
