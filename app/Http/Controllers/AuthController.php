...
class AuthController extends Controller  {
    ...
    public function login(Request $r,MfaService $mfa) {
        ...
        if(!$u||!Hash::check($d['password'],$u->password))return back()->withErrors(['login'=>'The supplied credentials are invalid.']);
        $mfa->issue($u);
    ...
    public function verifyMfa(Request $r,MfaService $mfa) {
        ...
        if(!$mfa->verify($u,$d['code']))return back()->withErrors(['code'=>'The code is invalid, expired, or has exceeded its attempt limit.']);
        Auth::login($u);
