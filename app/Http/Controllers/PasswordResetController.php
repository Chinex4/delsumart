...
class PasswordResetController extends Controller
{
    ...
    {
        return view('auth.reset-password', ['token' => $token, 'email' => $r->string('email')->value()]);
    }

    public function reset(Request $r)
    {
        $d = $r->validate(['token' => 'required', 'email' => 'required|email', 'password' => ['required', 'confirmed', PasswordRule::min(8)->mixedCase()->numbers()]]);
        $status = Password::reset($d, function ($user, $password) {
