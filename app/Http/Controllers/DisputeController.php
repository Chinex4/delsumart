...
class DisputeController extends Controller
{
    public function store(
        ...
        $dispute = DB::transaction(function () use ($r, $transaction, $d) {
            ...
            abort_unless(
                in_array($tx->status, ['paid_held', 'release_pending'], true),
                409,
            );
        ...
        $other =
            $r->user()->id === $transaction->buyer_id
                ? $transaction->seller
                : $transaction->buyer;
        ...
        return back()->with(
            'success',
            'Dispute opened. Release is paused while an administrator reviews it.',
        );
    }
}
