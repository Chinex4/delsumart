...
class DisputeController extends Controller  {
    public function store(Request $r,Transaction $transaction,FraudScoringService $fraud) {
        ...
        $dispute=DB::transaction(function()use($r,$transaction,$d) {
            $tx=Transaction::lockForUpdate()->findOrFail($transaction->id);abort_unless(in_array($tx->status,['paid_held','release_pending'],true),409);$d['complainant_id']=$r->user()->id;if($r->hasFile('evidence'))$d['evidence_path']=$r->file('evidence')->store('disputes/'.$tx->id,'local');$dispute=$tx->disputes()->create($d);$tx->update(['status'=>'disputed']);return $dispute;
        }
        );
