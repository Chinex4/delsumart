...
class FraudScoringService
{
    public function scoreUser(User $user): array
    {
        ...
        $sellerDisputes = Dispute::whereIn(
            'transaction_id',
            $user->sales()->select('id'),
        )->count();
        if ($sellerDisputes >= 3) {
        ...
        if ($highValue >= 3) {
            $add(
                10,
                'Three or more high-value listings created within 24 hours',
            );
        }
        $score = min($score, 100);
