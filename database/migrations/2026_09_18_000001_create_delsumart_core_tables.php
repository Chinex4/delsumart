...
return new class extends Migration
{
    public function up(): void
    {
        Schema::create('users', function (Blueprint $t) {
            ...
            $t->string('level', 20);
            $t->string('password');
            $t->enum('role', ['student', 'admin'])
                ->default('student')
                ->index();
            $t->enum('account_status', ['active', 'suspended'])->default(
                'active',
            );
            $t->rememberToken();
            $t->timestamps();
        ...
        Schema::create('listings', function (Blueprint $t) {
            ...
            $t->enum('status', ['active', 'sold', 'removed'])
                ->default('active')
                ->index();
            $t->timestamps();
        });
        ...
        Schema::create('disputes', function (Blueprint $t) {
            ...
            $t->foreignId('complainant_id')
                ->constrained('users')
                ->restrictOnDelete();
            $t->string('category');
            $t->text('details');
            $t->string('evidence_path')->nullable();
            $t->enum('status', ['open', 'under_review', 'resolved', 'rejected'])
                ->default('open')
                ->index();
            $t->text('resolution')->nullable();
            $t->foreignId('resolved_by')
                ->nullable()
                ->constrained('users')
                ->nullOnDelete();
        ...
        Schema::create('fraud_flags', function (Blueprint $t) {
            ...
            $t->enum('status', ['open', 'reviewed', 'dismissed'])
                ->default('open')
                ->index();
